...
from lattice.tools.file_safety import resolve_in_workspace


async def read_file(path: str, *, workspace: Path) -> str:
    target = resolve_in_workspace(path, workspace)
    return await asyncio.to_thread(target.read_text, encoding="utf-8")


async def write_file(path: str, content: str, *, workspace: Path) -> str:
    target = resolve_in_workspace(path, workspace)
...
async def edit_file(path: str, old: str, new: str, *, workspace: Path) -> str:
    target = resolve_in_workspace(path, workspace)
