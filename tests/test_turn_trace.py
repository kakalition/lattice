...
async def test_logging_turn_events_records_tools_and_skills(caplog) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="lattice.turn"):
        trace.log_begin(
            ...
            tools=["schedule/add", "interaction/todo"],
            skills=[("safe-shell", "shell skill")],
        )
        await trace.on_tool_start("schedule/add", {"reminder": "journal", "run_at": "x"})
        await trace.on_tool_end("schedule/add", "scheduled job-1")
        await trace.on_tool_start("skills/view", {"name": "weekly-review"})
        await trace.on_tool_end("skills/view", "body…")
        trace.log_end(outbound_text="done")
    text = "\n".join(r.message for r in caplog.records)
    assert "turn=abcd1234 BEGIN" in text
    assert "tools_offered" in text and "schedule/add" in text
    assert "skills_offered" in text and "safe-shell" in text
    assert "skill_meta" not in text
    assert "tool_start: schedule/add" in text
